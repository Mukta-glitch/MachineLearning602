...
for predictor_col in predictor_df.columns:
    ...
    if predictor_df[predictor_col].dtypes == 'object':
        ...
        print(predictor_col+ " TYPE OF PREDICTOR IS - "+predictor_type)
        fig = px.scatter(predictor_df,x=predictor_col,y=response_df['home_team_wins'],color=predictor_col)
        fig.update_layout(title=f'{predictor_col} vs home_team_wins', xaxis_title=predictor_col, yaxis_title='home_team_wins')
...
# separate response and predictor dataframes
X = predictor_df
...
# create a pipeline that includes preprocessing and a model of your choice
pipeline = Pipeline([
...
def plot_categorical_predictor_with_boolean_response(df, feature_names, y, response):
    ...
    file_name_cat_con = f"hw_5_plot/{feature_names}.html"
    fig.write_html(file_name_cat_con, include_plotlyjs="cdn")
...
# def plot_continuous_predictor_with_boolean_response(df, feature_names, y,response):
#     """Plot a distribution plot of a continuous predictor with a boolean response"""
#     df_plot = df.astype(float)
#     hist_data = [df_plot[response == 0][feature_names], df_plot[response == 1][feature_names]]
#     group_labels = ["Response=0", "Response=1"]
#     colors = ["slategray", "magenta"]
#     fig = ff.create_distplot(hist_data, group_labels, bin_size=10, colors=colors)
#     fig.update_layout(title="Continuous Predictor by Boolean Response")
#     file_name = f"hw_05_plot{feature_names}.html"
#     file_name_cat_con = f"hw_05_plot{feature_names}.html"
#     fig.write_html(file=file_name, include_plotlyjs="cdn")
#     fig.write_html(file=file_name_cat_con, include_plotlyjs="cdn")
...
def plot_categorical_predictor_with_continuous_response(feature, feature_names, y, response):
    ...
    fig.show()
    file_name = f"plot/hw_5_plot/cr_cat_{feature_names}.html"
    file_name_cat_con = f"hw_05_plot{feature_names}.html"
    fig.write_html(
        file=file_name,
...
for index, column in enumerate(X):
    ...
    print(feature)
    print("Column: "+column)
    function = predictor_response_map.get((predictor_type.lower(), response_type.lower()))
    # function = predictor_response_map.get[(predictor_type.lower(), response_type.lower())]
