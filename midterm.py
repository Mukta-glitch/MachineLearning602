...
def plot_hist(pred):
    pop_mean = [pred.mean()] * len(pred)
    data = [
        go.Histogram(x=pred, xbins={"size": len(pred.unique()) / 20}, name="Bin"),
        go.Scatter(x=pred[0:], y=pop_mean, mode="lines", name="Predictor Mean"),
    ]
    return
...
def plot_cont_cont(to_plot):
    ...
    for idx, column in enumerate(X.T):
        ...
        file_name = f"Midterm_{feature_name}_lin_plot.html"
...
def plot_cont_cat(to_plot):
    ...
    for idx, column in enumerate(X.T):
        ...
        file_name = f"Midterm_{feature_name}_cont_cat_area_plot.html"
        fig.write_html(file=file_name, include_plotlyjs="cdn")
...
def plot_cat_cat(to_plot):
    ...
    for idx, column in enumerate(X.T):
        ...
        file_name = f"Midterm_{feature_name}_cat_cat_violin_plot.html"
        fig.write_html(file=file_name, include_plotlyjs="cdn")
...
def plot_cat_cont(to_plot):
    ...
    for idx, column in enumerate(X.T):
        ...
        file_name = f"Midterm_{feature_name}__cat_cont_Scatter.html"
...
def plot_bool(to_plot):
    ...
    for idx, column in enumerate(X.T):
        ...
        file_name = f"Midterm_{feature_name}_bool_plot.html"
        fig.write_html(file=f"Midterm_{feature_name}log_plot.html", include_plotlyjs="cdn")
...
def main():
    ...
    cont_feature_df = pred_df[pred_df["Category"] == "continuous"]
    try:
        cont_feature_df = cont_feature_df.drop("target", axis=1, inplace=True)
    except Exception:
        pass

    cat_feature_df = pred_df[pred_df["Category"] != "continuous"]
    try:
        cat_feature_df = cat_feature_df.drop("target", axis=1, inplace=True)
    except Exception:
        pass

    binary_feature_df = pred_df[pred_df["Category"] == "binary"]
    try:
    ...
    # 2
    report_col_2 = (
        "CORRELATION CONTINOUS VS CONTINOUS",
    ...
    list_to_df(
        pd.DataFrame({"CORRELATION CONTINOUS VS CONTINOUS": corr_con_con_list}),
        report_df_2,
        "CORRELATION CONTINOUS VS CONTINOUS",
    )
    ...
    list_to_df(
        pd.DataFrame({"CORRELATION CONTINOUS VS CATEGORICAL": corr_con_cat_list}),
        report_df_2,
        "CORRELATION CONTINOUS VS CATEGORICAL",
    )
    ...
    temp_df = pd.DataFrame(
        corr_con_con_list, columns=["CORRELATION CONTINOUS VS CONTINOUS"]
    )
    ...
    temp_df = pd.DataFrame(
        corr_con_con_list, columns=["CORRELATION CONTINOUS VS CATEGORICAL"]
    )
    list_to_df(temp_df, report_df_2, "CORRELATION CONTINOUS VS CATEGORICAL")

    report_df_2 = report_df_2.sort_values(
        ["CORRELATION CONTINOUS VS CONTINOUS"], ascending=[False]
    )
    report_df_2 = report_df_2.drop(["target"])
    report_df_2.to_html(
        "REPORT 2" + datetime.now().strftime("%Y_%m_%d-%H_%M") + ".html"
    )
    ...
    with open(
        "REPORT 1" + datetime.now().strftime("%Y_%m_%d-%H_%M") + ".html", "w"
    ) as temp:
        temp.write(report)
